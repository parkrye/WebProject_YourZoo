...
/**
 * **돌아다니는** 범위. 놓는 자리(`PLACE_BOX`)보다 넓다.
 *
 * 둘을 하나로 두었더니 이동 범위가 놓는 자리에 묶여, 땅 동물이 손바닥만 한 띠
 * 안에서만 오르내렸다. 놓는 자리는 눈에 잘 띄는 한가운데여야 하지만
 * 돌아다니는 범위까지 거기 맞출 이유는 없다.
 */
export const ROAM_BOX: Record<Habitat, RoamBox> = {
  /*
    하늘은 **우리 전체**를 쓴다. 땅 위든 물 위든 날아서 지나갈 수 있다 —
    새가 공중의 절반에만 갇혀 있으면 우리 안을 도는 게 아니라 천장에 붙어 있게 된다.
    상단은 HUD(일자·시계·우리 이름)가 차지한다 — 0.04 로 두면 글자와 겹친다.
    아래는 물 로밍 박스와 같은 선까지다. 그보다 더 내리면 화면 밖으로 반쯤 나간다.

    내려와도 **착지하지는 않는다.** 렌더가 `SKY_HOVER` 만큼 띄우고,
    커지는 폭도 `SKY_PERSPECTIVE` 로 좁혀 땅짐승과 구분한다(`toRenderState`).
  */
  SKY: { x0: 0.04, x1: 0.96, y0: 0.10, y1: 0.96 },
  /*
    땅은 **물가 직전까지**다. 세 바이옴 모두 0.53 부터 물이 시작되는 0.70~0.72 까지가
    칠해진 땅이다. 그 위는 원경 수풀(초원)·언덕(사막)·먼 바다(설원)라
    거기에 세우면 공중에 뜬 것처럼 보인다.
  */
  LAND: { x0: 0.04, x1: 0.96, y0: 0.53, y1: 0.70 },
  // 0.72 는 물가 잔디 경계라 동물이 뭍에 걸쳐 보인다. 확실히 물속으로 내린다.
  WATER: { x0: 0.04, x1: 0.96, y0: 0.75, y1: 0.96 },
}

/**
 * 창고에서 꺼내 **놓을 수 있는** 자리. 점선 안내도 이 상자를 그린다.
 *
 * 이동 범위와 달리 좁게 유지한다. 놓자마자 화면 구석이나 물가에 걸쳐 있으면
 * 무엇을 놓았는지 확인할 수가 없다 — 놓은 다음 스스로 넓게 돌아다니는 것과
 * 처음부터 가장자리에 놓이는 것은 다른 일이다.
 */
export const PLACE_BOX: Record<Habitat, RoamBox> = {
  SKY: { x0: 0.04, x1: 0.96, y0: 0.10, y1: 0.26 },
  LAND: { x0: 0.04, x1: 0.96, y0: 0.53, y1: 0.68 },
  WATER: { x0: 0.04, x1: 0.96, y0: 0.75, y1: 0.96 },
}
...
/**
 * 펜스 Y 오프셋 (화면 높이 비율). 상세보기에서 펜스를 내려 우리 안을 노출한다.
 *
 * 펜스 그림은 0.42 부터 기둥이 서고 **0.59 부터는 꽉 막힌 벽**이다.
 * 땅 로밍 박스가 0.52~0.66, 물이 0.75~0.96 이므로 0.2 만 내리면
 * 벽 상단이 0.79 에 걸려 물에 사는 동물이 여전히 반쯤 가렸다.
 * 0.34 면 벽 상단이 0.93 까지 내려가 둘 다 보이고, 기둥(0.76~)은 남아
 * 여기가 우리 안이라는 경계는 그대로 읽힌다.
 */
export const FENCE_OFFSET_ZOO = 0
export const FENCE_OFFSET_DETAIL = 0.34
...
/**
 * 하늘 동물의 원근 배율. 땅·물보다 **폭이 좁다.**
 *
 * 나는 것은 땅을 밟는 것보다 카메라에서 멀다 — 같은 y 로 내려와도 물에 있는
 * 물고기만큼 커지면 물속에 빠진 것처럼 보인다. 전혀 안 커지게 두는 것도 틀렸다.
 * 그러면 화면을 가로지를 때 깊이가 사라져 배경에 붙은 스티커가 된다.
 */
export const SKY_PERSPECTIVE = { near: 1.06, far: 0.94 } as const

/**
 * 하늘 동물을 그릴 때 발밑 기준선에서 띄우는 높이 (정규화).
 *
 * 하늘도 이제 땅과 물 위를 지나므로, 안 띄우면 물가에서 물고기와 같은 선에 발을
 * 딛고 선다. 이만큼 떠 있어야 **내려앉은 것이 아니라 그 위를 나는 것**으로 읽힌다.
 */
export const SKY_HOVER = 0.035
