...
│
├─ assets/
│  ├─ images/                  ★ 전처리된 에셋 (bg/ · sprite/)
│  ├─ manifest.ts              ★ 에셋 import + 그리드 스펙 (docs/01 의 구현체)
...
| # | 리스크 | 영향 | 대응 |
|---|---|---|---|
| R1 | **에셋 알파 오판** | 글자·스프라이트가 갉아먹히거나 검은 판이 찍힘 | 알파는 눈이 아니라 채널로 확인. 전처리는 `scripts/prepare-assets.py` 에서 한 번만. `?dev=assets` 로 눈 검증 |
| R2 | 팝업 프레임이 비정형 | 팝업 사용 불가 | 9-슬라이스만 사용해 리스크 자체를 회피 |
...
아래 다섯은 넣는 데 몇 줄이면 되고 빠뜨렸을 때 대가가 크다.

| 막는 것 | 방법 |
|---|---|
| 비밀번호 유출 | scrypt + 계정마다 다른 소금. 원문도 가역 형태도 남기지 않는다 |
| 경로 조작 | 아이디·그림 id 를 정규식으로 좁힌다. 파일명이 되기 때문이다 |
| 무제한 대입 | 같은 (IP, 아이디) 5분에 10회. 넘으면 429 |
| 무제한 본문 | 24MB 상한, 배열 길이·문자열 길이 상한 |
| 위장 업로드 | PNG 시그니처를 확인한다. 확장자만 믿으면 아무거나 올라온다 |

그 밖에:

- **응답 봉투가 하나다.** 성공 `{ ok: true, ... }` / 실패 `{ ok: false, code, message }`.
  `code` 는 기계가 보고 `message` 는 화면에 그대로 띄운다.
- **경로에 버전이 있다** (`/api/v1`). 버전 없는 경로는 404 로 거절해,
  규격을 바꿀 때 옛 클라이언트가 조용히 깨지지 않게 한다.
- **`Content-Type: application/json` 을 요구한다.** 형식 때문만이 아니다 —
  이걸 요구하면 폼 전송으로 위장한 교차 출처 요청이 프리플라이트에 걸린다.
- **쿠키를 쓰지 않는다.** 토큰은 `x-zoo-token` 헤더로만 간다. CSRF 표면이 없다.
- **아이디가 없는 것과 비밀번호가 틀린 것을 구분해 알리지 않는다.**
  어느 아이디가 존재하는지 떠보는 통로가 된다.
- **토큰 비교도 시간을 흘리지 않는다** (`timingSafeEqual`). 로그인마다 새로 발급하고 30일이면 만료된다.
- `X-Content-Type-Options: nosniff`, `Cache-Control: no-store`, `Referrer-Policy: no-referrer`.

비회원 아이디에는 계정이 없으므로 그냥 통과시킨다. 무작위 7자리라 남이 맞힐 일이 없고,
막아 봐야 지킬 게 없다.
